...
# Stripe Price IDs — copy from Stripe dashboard after creating the products
STRIPE_PRICE_PACK_STARTER=price_...    # $9.99 one-time, 50 credits
STRIPE_PRICE_PACK_STANDARD=price_...   # $19.99 one-time, 120 credits
STRIPE_PRICE_PACK_PRO=price_...        # $39.99 one-time, 300 credits
...
- `analyze.py` — `POST /analyze` (pre-checks sync, heavy work async; returns `{"job_id": "..."}` immediately); `GET /analyze/status/{job_id}` (poll for result; requires `x-user-id`, returns 403 if job belongs to a different user)
...
Every user starts with 0 credits. Credits are spent when an analysis runs. Cost is **flat 10 credits per analysis** regardless of comment count — `credits_for_count()` in `app/config.py` always returns `10`.
...
- `checkout.session.completed` → adds credits for all three packs (driven by `STRIPE_CREDITS` in `app/config.py`)

Only `checkout.session.completed` is handled. The handler is **idempotent** — it checks for an existing `CreditTransaction` with the same `stripe_session_id` before awarding credits, so Stripe retries cannot double-award. `CreditTransaction.stripe_session_id` also has a sparse unique index in MongoDB as a second layer of protection.

Subscription products (`sub_starter`, `sub_pro`) and `invoice.payment_succeeded` have been removed.
...
`POST /analyze` runs pre-checks synchronously (auth, video ID parsing, `fetch_video` for metadata, credit check), then spawns a `threading.Thread` for the slow work (comment fetch + AI analysis) and returns `{"job_id": "<uuid>"}` immediately. Job state is kept in the module-level `_jobs` dict (`{status, result, error, user_id}`; status values: `pending | running | done | failed`). A `threading.Semaphore(3)` caps concurrent jobs — returns 503 if all slots are taken.

The semaphore is always released — either in the thread's `finally` block on success, or in the handler's own `finally` block via a `thread_started` flag if any exception fires before the thread launches. This prevents slot leaks from unexpected errors (MongoDB timeouts, YouTube client failures, etc.).

`GET /analyze/status/{job_id}` verifies the caller's `x-user-id` matches the job owner before returning anything, and **deletes the job from `_jobs`** on `done`/`failed` to prevent unbounded memory growth.

## Logging

The app uses Python's standard `logging` module, configured once in `app/main.py` with format:
```
%(asctime)s %(levelname)s %(name)s %(message)s
```
Each module has its own named logger (`logging.getLogger(__name__)`). Key context fields — `user_id`, `video_id`, `job_id`, `provider`, `duration_s`, `credits_spent` — are included inline in log messages so failures can be traced in Railway logs by grepping a single identifier.

Key log events: `job_start`, `job_cached`, `job_done`, `job_failed` (analyze router); `analysis_start`, `analysis_done` (analyzer service); `gemini_json_parse_error` (gemini AI); `webhook_credits_awarded`, `webhook_duplicate`, `webhook_missing_metadata` (payments router).
...
- [ ] Create 3 products in [Stripe Dashboard](https://dashboard.stripe.com/products):
  - **Starter Pack** — one-time, $9.99, 50 credits
  - **Standard Pack** — one-time, $19.99, 120 credits
  - **Pro Pack** — one-time, $39.99, 300 credits
- [ ] Copy the Price IDs into Railway env vars: `STRIPE_PRICE_PACK_STARTER`, `STRIPE_PRICE_PACK_STANDARD`, `STRIPE_PRICE_PACK_PRO`
...
- [x] **Structured logging** — Python `logging` module with user_id, video_id, job_id, duration_s on every key event
- [x] **Semaphore leak** — `thread_started` flag ensures the semaphore slot is always released even if pre-checks raise unexpected exceptions
- [x] **Webhook idempotency** — `stripe_session_id` guard prevents duplicate credit awards on Stripe retries
- [x] **Job ownership** — `GET /analyze/status/{job_id}` verifies `x-user-id` matches the job owner
